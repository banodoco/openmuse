...
export class SupabaseDatabase extends BaseDatabase {
  ...
  async getAllEntries(approvalFilter: 'all' | 'curated' = 'all'): Promise<VideoEntry[]> {
    ...
    // --- Execute the query with a timeout safeguard ---
    const QUERY_TIMEOUT_MS = 12000; // 12 seconds (slightly longer than the UI timeout)

    const queryStart = performance.now();

    // Prepare the actual Supabase call (do not execute yet)
    const queryPromise = query
      .eq('type', 'video')
      .order('created_at', { ascending: false });

    // Race the Supabase promise against a timeout – if either resolves/rejects first, that result wins.
    ...
    ]) as { data: any; error: any }; // Cast for simplicity – handled below

    const queryDuration = (performance.now() - queryStart).toFixed(2);
    this.logger.log(`[getAllEntries] Supabase query (or timeout) resolved in ${queryDuration} ms`);
